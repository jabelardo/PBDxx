...
typedef struct pbd_element_vtable {
    ...
    int (*to_buffer)(pbd_conf conf, const struct pbd_element* e, char** buffer, size_t* size);
    int (*from_buffer)(pbd_conf conf, struct pbd_element* e, const char* buffer, pbd_type_id type_id,  size_t* read_bytes);
    void (*free)(pbd_conf conf, const struct pbd_element* e);
} pbd_element_vtable;
